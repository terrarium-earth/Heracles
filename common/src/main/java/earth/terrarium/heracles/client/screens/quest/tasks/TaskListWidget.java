...
public class TaskListWidget extends AbstractContainerEventHandler implements Renderable {
    ...
    private final List<QuestTaskWidget> widgets = new ArrayList<>();
    ...
    public void update(List<QuestTask<?, ?>> tasks) {
        List<QuestTaskWidget> inProgress = new ArrayList<>();
        List<QuestTaskWidget> completed = new ArrayList<>();
        for (var task : tasks) {
            TaskProgress taskProgress = this.progress.getTask(task.id());
            QuestTaskWidget widget = QuestTaskWidgets.create(task, taskProgress);
            if (widget != null) {
        ...
        if (!inProgress.isEmpty()) {
            this.widgets.add(new TaskHeadingWidget(IN_PROGRESS, 0xFF5691FF));
            this.widgets.addAll(inProgress);
        }
        if (!completed.isEmpty()) {
            this.widgets.add(new TaskHeadingWidget(COMPLETED, 0xFF04CB40));
    ...
    public void render(PoseStack pose, int mouseX, int mouseY, float partialTick) {
        ...
        try (var scissor = RenderUtils.createScissorBoxStack(new ScissorBoxStack(), Minecraft.getInstance(), pose, x, y, width, height)) {
            for (QuestTaskWidget widget : this.widgets) {
