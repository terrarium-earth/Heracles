...
public record ChangedDimensionTask(
    String id, ResourceKey<Level> from, ResourceKey<Level> to
) implements QuestTask<Pair<ResourceKey<Level>, ResourceKey<Level>>, ByteTag, ChangedDimensionTask> {
    ...
    private static class Type implements QuestTaskType<ChangedDimensionTask> {
        ...
        public Codec<ChangedDimensionTask> codec(String id) {
            ...
            ).apply(instance, (i, from, to) -> new ChangedDimensionTask(i, from.orElse(null), to.orElse(null))));
        }
    }
}
