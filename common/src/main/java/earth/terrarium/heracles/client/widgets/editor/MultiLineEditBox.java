...
public class MultiLineEditBox extends AbstractScrollWidget {
	...
	protected void renderContents(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
		...
		if (value.isEmpty() && this.isFocused()) {
			...
			if (showCursor) {
				graphics.fill(lineX, lineY, lineX + 1, lineY + 9, 0xFF000000);
			}
		} else {
			...
			for(MultilineTextField.StringView view : this.field.lines()) {
				...
				if (showCursor && cursor >= view.beginIndex() && cursor <= view.endIndex()) {
					if (shouldRender) {
						int cursorX = graphics.drawString(this.font, this.highlighter.highlight(value.substring(view.beginIndex(), cursor)), lineX, lineY, 0x555555, false);
						if (isEndOfLine) {
							cursorX++;
						}
						graphics.fill(cursorX - 1, lineY - 1, cursorX, lineY - 1 + 9, 0xFF000000);
						graphics.drawString(this.font, this.highlighter.highlight(value.substring(cursor, view.endIndex())), cursorX, lineY, 0x555555, false);
					}
				} else if (shouldRender) {
					graphics.drawString(this.font, this.highlighter.highlight(value.substring(view.beginIndex(), view.endIndex())), lineX, lineY, 0x555555, false);
				}
