...
public record ServerboundUpdateGroupOrderPacket(List<String> groupOrders) implements Packet<ServerboundUpdateGroupOrderPacket> {
    ...
    private static class Type implements ServerboundPacketType<ServerboundUpdateGroupOrderPacket> {
        ...
        public ResourceLocation id() {
            return Heracles.id("update_group_order");
        }

        @Override
        public void encode(ServerboundUpdateGroupOrderPacket message, RegistryFriendlyByteBuf buffer) {
            buffer.writeCollection(message.groupOrders(), FriendlyByteBuf::writeUtf);
        }

        @Override
        public ServerboundUpdateGroupOrderPacket decode(RegistryFriendlyByteBuf buffer) {
        ...
        public Consumer<Player> handle(ServerboundUpdateGroupOrderPacket message) {
            return (player) -> {
                if (player.hasPermissions(2)) {
                    ...
                    QuestHandler.groupsOrder().addAll(message.groupOrders());
                    QuestHandler.saveGroups();
                    if (player.getServer() != null) {
                        NetworkHandler.CHANNEL.sendToPlayers(
                            new SyncGroupOrderPacket(message.groupOrders()),
